...
test.describe('Matters API - Advanced Search Functionality', () => {
  ...
  test('should search with partial word (beginning)', async ({ request }) => {
    ...
    // Verify at least one result contains the matching term
    if (data.data.length > 0) {
      const hasLitigation = data.data.some((matter: Matter) => {
        const searchableText = JSON.stringify(matter.fields).toLowerCase();
        return searchableText.includes('litig');
      });
      expect(hasLitigation).toBe(true);
    }
  });
  ...
  test('should search with partial word (middle)', async ({ request }) => {
    ...
    // Verify results contain the search substring
    if (data.data.length > 0) {
      const hasCompliance = data.data.some((matter: Matter) => {
        const searchableText = JSON.stringify(matter.fields).toLowerCase();
        return searchableText.includes('mplianc');
      });
  ...
  test('should handle search with extra spaces', async ({ request }) => {
    ...
    const hasMatch = data.data.some((matter: Matter) => {
      const searchableText = JSON.stringify(matter.fields).toLowerCase();
      return searchableText.includes('contract') || searchableText.includes('review');
    });
  ...
  test('should handle search with parentheses', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Likely no matches for this specific pattern
    expect(data.total).toBeGreaterThanOrEqual(0);
  });
  ...
  test('should handle search with quotes', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Should find Contract Review matters (quotes ignored)
    expect(data.total).toBeGreaterThan(0);
  });
  ...
  test('should handle search with apostrophe', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Unlikely to match but should not error
    expect(data.total).toBeGreaterThanOrEqual(0);
  });
  ...
  test('should handle search with backslash', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Unlikely pattern - expect zero results
  ...
  test('should handle search with SQL injection attempt', async ({ request }) => {
    ...
    expect(response.ok()).toBeTruthy();
    const data = await response.json();
    // Should safely handle SQL injection attempt - no results expected
    expect(data).toHaveProperty('data');
    expect(data.total).toBe(0);
  ...
  test('should handle search with XSS attempt', async ({ request }) => {
    ...
    expect(response.ok()).toBeTruthy();
    const data = await response.json();
    // Should safely handle XSS attempt - no results expected
    expect(data).toHaveProperty('data');
    expect(data.total).toBe(0);
    expect(data.data.length).toBe(0);
  });
  ...
  test('should handle search with percent sign (LIKE wildcard)', async ({ request }) => {
    ...
    // May find results matching 'Matter' (ignoring %)
    expect(data).toHaveProperty('data');
    expect(data.total).toBeGreaterThanOrEqual(0);
  });
  ...
  test('should handle search with underscore (LIKE wildcard)', async ({ request }) => {
    ...
    expect(response.ok()).toBeTruthy();
    const data = await response.json();
    // Should treat _ as literal character, not wildcard - no results expected
    expect(data).toHaveProperty('data');
    expect(data.total).toBe(0);
  });
  ...
  test('should handle very long search string', async ({ request }) => {
    ...
    expect(response.ok()).toBeTruthy();
    const data = await response.json();
    // Should handle long search strings without error and find Contract Review matters
    expect(data).toHaveProperty('data');
    expect(data.total).toBeGreaterThan(0);
  });
  ...
  test('should handle search with unicode characters', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Unlikely to have unicode in test data
    expect(data.total).toBeGreaterThanOrEqual(0);
  });
  ...
  test('should handle search with emoji', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Search will find 'Matter' (emoji is ignored)
    expect(data.total).toBeGreaterThan(0);
  });
  ...
  test('should handle search with newline characters', async ({ request }) => {
    ...
    const data = await response.json();
    expect(data).toHaveProperty('data');
    // Should normalize whitespace and find Contract Review
    expect(data.total).toBeGreaterThan(0);
  });
